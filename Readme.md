...
**Guide version: v42** — The full moOde library cleanup guide (`linux-audio-moode-cleanup-guide.md`). Current version; supersedes v41. Documentation fix: Step 5 (Reapply ReplayGain) clarified to cover every loudgain-supported format (FLAC, MP3, M4A/MP4, OGG, Opus, APE, WavPack, Speex) — the script was always format-agnostic; only the prose said "FLAC" (2026-09-21, see the change log). The no-`.sh` conversion is complete as of v41 (2026-09-20): every embedded script — Steps 1, 1B, 2A–2E, 3, 4, 5, 6–10, 15a–15d and the Preflight — is extensionless Python. Earlier notable addition retained from v32: optional procedure 15d "Ignore-Content Certification", which integrity-tests every audio file inside every `Ignore` folder and writes a self-contained `Ignore.sha512sums.txt` per folder.
