...
This document is a technical guide and automated script workflow for auditing, repairing, and standardizing Moode Audio–compatible music libraries on Linux systems, focusing on integrity testing, metadata cleanup, container repair, and volume normalization.

It is intended for users who maintain large music collections and want a consistent process for correcting metadata issues, verifying file integrity, and preparing libraries for reliable playback on Moode Audio and for long-term archival.

This guide originated as a FLAC-only cleanup workflow. It has since expanded in scope to cover the range of formats Moode Audio supports for playback — FLAC, MP3, M4A/AAC, WavPack, and others — recognizing that a real-world library rarely consists of a single format. Not every step currently has equivalent coverage across all formats; see the Format Support Status table in Section 04 for a per-step breakdown of what's implemented versus what's still FLAC-only pending redesign.
...
Note: This list reflects tools required by the steps currently implemented for multi-format use. Additional format-specific tagging utilities — for example, tools capable of reading and writing ID3v2 TXXX frames (MP3), iTunes-style freeform atoms (M4A), or APEv2 items (WavPack) — will be added here as the pending steps in Section 04 are redesigned for those formats.
...
This guide is built on four core engineering principles:
...
* Format-Aware Processing: Each audio format stores metadata and container structure differently. Rather than forcing a single FLAC-centric method across all formats, each processing step is designed — or, where not yet complete, will be redesigned — around the native tooling appropriate to that format's own tag and container system.
...
1. Initial Integrity Test: Establish a baseline of existing issues, testing each file using the method appropriate to its format.
2. Deduplicate Metadata: Remove redundant metadata tag entries.
3. Container Rebuild: Strip invalid metadata headers without re-encoding audio streams.
4. Post-Rebuild Verification: Repeat the integrity test to validate structural container fixes.
5. ReplayGain Restoration: Recalculate and reapply volume normalization.
...
This guide is in transition from a FLAC-only workflow to full Moode-compatible multi-format coverage. The table below reflects current status as of this revision:


| Step | Step Name                  | Formats                                      | Status                                |
|------|----------------------------|----------------------------------------------|---------------------------------------|
| 1    | Initial Integrity Test     | FLAC, MP3, M4A, OGG, Opus, WAV, AIFF         | Complete                              |
| 2    | Deduplicate Metadata       | FLAC only                                    | Pending — per-format tag tools needed |
| 3    | Container Rebuild          | FLAC only                                    | Pending — FLAC-specific pathology     |
| 4    | Post-Rebuild Verification  | FLAC only                                    | Widen (mirror Step 1)                 |
| 5    | ReplayGain Restoration     | FLAC/MP3/M4A/OGG/Opus/MP4/AAC/APE/WV/MPC/Spx | Complete                              |
| 6    | Final Integrity Validation | FLAC only                                    | Widen (mirror Step 1)                 |
| 7    | Remove Loose Files         | Format-agnostic                              | Complete                              |
| 8    | Final Integrity Test       | FLAC only                                    | Widen (mirror Step 1)                 |
| 13a  | Strip Problematic Metadata | FLAC only                                    | Pending — no generic equivalent       |
...
| 13d  | Deep Repair (Re-encode)    | FLAC only                                    | Pending — FLAC-specific corruption    |
| 14   | Generate Checksums         | FLAC only                                    | See SHA-512 repo                      |
...
* All image files not .jpg converted to .jpg format and saved as Cover.jpg ONLY. 
...
for d in "${dirs[@]}"; do
    ...
    if [ -z "$art_file" ]; then
        ...
        echo "[$i/$total] ERROR: $label :: No Moode-standard cover image found (checked: ${COVER_CANDIDATES[*]})" >> "$LOGDIR/artwork_embedding_errors.log"
        continue
    fi
